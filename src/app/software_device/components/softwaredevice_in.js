...
import { getSoftwareInstalledSummaryData } from '../datasoftware_in';
import { getDevicesData } from '../../lib/device';
import { getAllSoftwareData } from '../../lib/software';

export default async function SoftwareDeviceInstalled() {
  try {
    // 1. ดึงข้อมูลพร้อมกัน
    ...
    // 3. รวบรวม deviceId ที่มี Software Installed อย่างน้อย 1 ตัว (unique)
    ...
    [...devicesWithInstalled].forEach(id => {
    ...
    const { mainStats } = getSoftwareInstalledSummaryData(counts);
    ...
    console.error("❌ SOFTWAREDEVICE_IN Error:", error);
    return <div className="p-4 text-red-500">Failed to load software installed device summary.</div>;
  }
}